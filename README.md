...
Compact, deduplicated, ranked interpretation. Optimized for:
- LLM consumption
- Downstream processing
- Human reading

Typically 80-90% smaller than raw output.
...
raw engine outputs
    ↓
candidate extraction (per engine)
    ↓
canonical tag normalization
    ↓
deduplication by surface form + morphology
    ↓
ranking by engine agreement + specificity
    ↓
limit alternatives (top 3 per word)
    ↓
limit morph tags (top 4 per analysis)
    ↓
normalized output
```

This turns a ~100KB raw output into a ~17KB normalized output while preserving all linguistic information.
